...
int on_new_session_callback(struct xio_session *xio_session,
	...
	ServerSession *jxio_session = new ServerSession(xio_session, portal,
			portal->get_ctx_class());
...
BULLSEYE_EXCLUDE_BLOCK_END
		LOG_DBG("xio_modify_session for session=%p to %p", xio_session,
			jxio_session);

	char* buf = ctx->get_buffer();
	int sizeWritten = ctx->events.writeOnNewSessionEvent(buf, portal,
			jxio_session, req);
...
int on_msg_send_complete_callback(struct xio_session *xio_session,
	...
	LOG_TRACE("got on_msg_send_complete_callback for msg=%p",
			msg->user_context);

	//must release the message
	Msg *msg_from_pool = (Msg*) msg->user_context;
	msg_from_pool->release_to_pool();
	LOG_TRACE("finished on_msg_send_complete_callback for msg=%p",
			msg->user_context);
	return 0;
}
...
int on_msg_callback_server(struct xio_session *xio_session, struct xio_msg *msg,
	...
	LOG_TRACE(
			"on_msg_callback portal=%p, num_iov=%d, len: in=%d out=%d, msg=%p",
			msg->user_context, msg->in.data_iov.nents, msg_in_size,
			msg_out_size, msg);
	...
	if (msg->user_context == NULL) {
		...
		need_copy = true;
		msg_from_pool = ctx->msg_pools.get_msg_from_pool(msg_in_size,
				msg_out_size);
	} else {
	...
	char* buf = ctx->get_buffer();
	int sizeWritten = ctx->events.writeOnRequestReceivedEvent(buf,
			msg->user_context, msg_in_size, msg_out_size, jxio_session);
...
int on_msg_error_callback_server(struct xio_session *xio_session,
		enum xio_status error, enum xio_msg_direction direction,
		struct xio_msg *msg, void *cb_prv_data) {
	LOG_DBG(
			"got on_msg_error_callback for msg=%p, direction=%d. error status is %d",
			msg->user_context, direction, error);
	ServerSession *jxio_session = (ServerSession*) cb_prv_data;
	...
	int sizeWritten = ctx->events.writeOnMsgErrorEventServer(buf,
			msg->user_context, jxio_session, error);
	ctx->done_event_creating(sizeWritten);
...
int on_session_event_callback_server(struct xio_session *xio_session,
	...
	LOG_DBG(
			"got on_session_event_callback. event=%d, reason=%d, cb_prv_data=%p, session=%p, conn=%p",
			event_data->event, event_data->reason, cb_prv_data, xio_session,
			event_data->conn);
	BULLSEYE_EXCLUDE_BLOCK_START
	if (cb_prv_data == NULL) {
		LOG_ERR(
				"cb_prv_data is NULL making jxio_session of type ServerSession NULL");
		return 1;
	}
	BULLSEYE_EXCLUDE_BLOCK_END
	ServerSession *jxio_session = (ServerSession*) cb_prv_data;
	ServerPortal *portal = jxio_session->get_portal_session_event(
			event_data->conn_user_context, event_data->conn, event_data->event);
	Context *ctx = portal->ctxForSessionEvent(event_data, jxio_session);
	if (ctx) {
		char* buf = ctx->get_buffer();
		int sizeWritten = ctx->events.writeOnSessionErrorEvent(buf,
				jxio_session, event_data);
		ctx->done_event_creating(sizeWritten);
...
int on_buffer_request_callback(struct xio_msg *msg, void *cb_user_context) {
	LOG_TRACE("got on_buffer_request_callback for msg=%p, user_context=%p", msg,
			cb_user_context);
	...
	const int msg_in_size = get_xio_msg_in_size(msg);
	const int msg_out_size = get_xio_msg_out_size(msg);
	Msg* msg_from_pool = ctx->msg_pools.get_msg_from_pool(msg_in_size,
			msg_out_size);
