...
public:
	...
	inline xio_context* get_xio_context() {
		return ctx;
	}
	;
	...
	inline int get_events_count() {
		return events_queue.get_count();
	}
	;
	void reset_events_counters() {
		this->events_queue.reset();
	}
	;

	inline int scheduled_events_count() {
		return this->scheduled_events_queue.size();
	}
	;
