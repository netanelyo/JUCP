...
Msg::Msg(char* buf, struct xio_mr* xio_mr, int in_buf_size, int out_buf_size,
		MsgPool* pool) :
		in_buf_size(in_buf_size), out_buf_size(out_buf_size), xio_mr(xio_mr), pool(
				pool), buf(buf), buf_out(buf + in_buf_size) {
...
int Msg::send_response(const int out_size) {
	MSG_LOG_TRACE("sending %d bytes, xio_msg is %p", out_size,
			this->get_xio_msg());
	...
	if (ret)
		MSG_LOG_DBG("Got error from sending xio_msg: '%s' (%d)",
...
void Msg::dump(struct xio_msg *xio_msg) {
	...
	if (xio_msg->type == XIO_MSG_TYPE_REQ)
		...
		MSG_LOG_TRACE("response:%p, serial number:%ld", xio_msg->request,
				((xio_msg->request) ? xio_msg->request->sn : -1));

	MSG_LOG_TRACE("in header: length:%d, address:%p",
	...
	for (int i = 0; i < xio_msg->in.data_iov.nents; i++)
		MSG_LOG_TRACE("in data[%d]: length:%d, address:%p, mr:%p", i,
				xio_msg->in.data_iov.sglist[i].iov_len,
				xio_msg->in.data_iov.sglist[i].iov_base,
				xio_msg->in.data_iov.sglist[i].mr);

	MSG_LOG_TRACE("out header: length:%d, address:%p",
	...
	for (int i = 0; i < xio_msg->out.data_iov.nents; i++)
		MSG_LOG_TRACE("out data[%d]: length:%d, address:%p, mr:%p", i,
				xio_msg->out.data_iov.sglist[i].iov_len,
				xio_msg->out.data_iov.sglist[i].iov_base,
				xio_msg->out.data_iov.sglist[i].mr);
	MSG_LOG_TRACE("*********************************************");
}
#if _BullseyeCoverage
