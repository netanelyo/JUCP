...
Client::Client(const char* url, long ptrCtx) {
	...
	if (session == NULL) {
		CLIENT_LOG_ERR(
				"Error in creating session for Context=%p (errno=%d '%s')",
				this->ctx_class, xio_errno(), xio_strerror(xio_errno()));
		throw std::bad_alloc();
	}
...
bool Client::create_connection() {
	...
	BULLSEYE_EXCLUDE_BLOCK_START
	if (con == NULL) {
		CLIENT_LOG_ERR("Error in creating connection for Context=%p",
				this->ctx_class);
...
bool Client::close_connection() {
	...
	if (xio_disconnect(this->con)) {
		CLIENT_LOG_ERR("Error xio_disconnect failure: '%s' (%d)",
...
Context* Client::ctxForSessionEvent(struct xio_session_event_data * event,
		struct xio_session *session) {
	Context *ctx;
	switch (event->event) {
	case XIO_SESSION_CONNECTION_CLOSED_EVENT: //event created because user on this side called "close"
		CLIENT_LOG_DBG(
				"got XIO_SESSION_CONNECTION_CLOSED_EVENT. Reason=%s (%d)",
				xio_strerror(event->reason), event->reason);
		this->is_closing = true;
		return this->get_ctx_class();

	case XIO_SESSION_CONNECTION_TEARDOWN_EVENT:
		CLIENT_LOG_DBG(
				"got XIO_SESSION_CONNECTION_TEARDOWN_EVENT. Reason=%s (%d)",
				xio_strerror(event->reason), event->reason);
	...
	case XIO_SESSION_CONNECTION_REFUSED_EVENT:
		CLIENT_LOG_DBG(
				"got XIO_SESSION_CONNECTION_REFUSED_EVENT. Reason=%s (%d)",
				xio_strerror(event->reason), event->reason);
		this->is_closing = true;
		return this->get_ctx_class();
	...
	case XIO_SESSION_CONNECTION_DISCONNECTED_EVENT: //event created "from underneath"
		CLIENT_LOG_DBG(
				"got XIO_SESSION_CONNECTION_DISCONNECTED_EVENT. Reason=%s (%d)",
				xio_strerror(event->reason), event->reason);
		this->is_closing = true;
		return this->get_ctx_class();
	...
	case XIO_SESSION_TEARDOWN_EVENT:
		...
		if (!this->is_closing) {
			CLIENT_LOG_ERR(
					"Got session teardown without getting connection/close/disconnected/rejected/refused");
		}
		//the event should also be written to buffer to let user know that the session was closed
		BULLSEYE_EXCLUDE_BLOCK_START
		if (xio_session_destroy(session)) {
			CLIENT_LOG_ERR("Error xio_session_close failure: '%s' (%d) ",
					xio_strerror(xio_errno()), xio_errno());
		}
		BULLSEYE_EXCLUDE_BLOCK_END
	...
	case XIO_SESSION_CONNECTION_ERROR_EVENT:
		CLIENT_LOG_DBG("got XIO_SESSION_CONNECTION_ERROR_EVENT. Reason=%s (%d)",
				xio_strerror(event->reason), event->reason);
		close_connection();
		return NULL;
	...
	default:
		CLIENT_LOG_WARN("UNHANDLED event: got event '%s' (%d)",
				xio_session_event_str(event->event), event->event);
		return this->get_ctx_class();
	BULLSEYE_EXCLUDE_BLOCK_END
}
}

int Client::send_msg(Msg *msg, const int out_size, const int in_size,
	const bool is_mirror) {
if (this->is_closing) {
	CLIENT_LOG_DBG(
			"attempting to send a message while client session is closing");
	return XIO_E_SESSION_DISCONNECTED;
}
CLIENT_LOG_TRACE("sending msg=%p, out_size=%d, in_size=%d", msg, out_size,
		in_size);
...
if (ret_val) {
	CLIENT_LOG_TRACE("Error in sending xio_msg: '%s' (%d) (ret_val=%d)",
			xio_strerror(xio_errno()), xio_errno(), ret_val);
	return xio_errno();
}
return 0;
}
