...
Context::Context(size_t events_queue_size) :
		events_queue(events_queue_size) {
	CONTEXT_LOG_DBG("CTOR start");

	staging_scheduled_event_buffer = new char[(sizeof(scheduled_event_t)
			+ EVENTQUEUE_HEADROOM_BUFFER)];
	staging_scheduled_event_in_use = false;

	this->ctx = xio_context_create(NULL, 0, -1);
	BULLSEYE_EXCLUDE_BLOCK_START
	if (ctx == NULL) {
		CONTEXT_LOG_ERR("ERROR on xio_context_create() (errno=%d '%s')",
...
int Context::run_event_loop(long timeout_micro_sec) {
	...
	if (scheduled_events_count() == 0) {
		...
		if (timeout_micro_sec == -1) {
			...
			CONTEXT_LOG_TRACE(
					"before ev_loop_run. requested timeout is %d msec",
					timeout_msec);
		}
	...
	CONTEXT_LOG_TRACE("after ev_loop_run. there are %d events",
			get_events_count());
...
void Context::done_event_creating(int size_written) {
	if (staging_scheduled_event_in_use == true) {
		...
		memcpy(scheduled_event.queued_event, staging_scheduled_event_buffer,
				size_written);
...
void Context::scheduled_events_add(scheduled_event_t& scheduled_event) {
	CONTEXT_LOG_TRACE("adding scheduled event (queue size = %d)",
...
int Context::scheduled_events_process() {
	...
	while (scheduled_events_count() > 0
			&& ((events_queue_buf = events_queue.get_buffer_offset()) != NULL)) {

		scheduled_event_t scheduled_event =
				this->scheduled_events_queue.front();

		// Write internal events to event queue
		memcpy(events_queue_buf, scheduled_event.queued_event,
				scheduled_event.size);
