...
int Events::writeOnSessionErrorEvent(char *buf, void *ptrForJava,
	...
	event->event_specific.session_error.error_reason = htonl(
			event_data->reason);
	this->size = sizeof(struct event_session_error)
			+ sizeof((queued_event_t *) 0)->type
			+ sizeof((queued_event_t *) 0)->ptr;
	return this->size;
}

int Events::writeOnSessionEstablishedEvent(char *buf, void *ptrForJava,
		struct xio_session *session, struct xio_new_session_rsp *rsp) {
	struct queued_event_t* event = (struct queued_event_t*) buf;
	event->type = htonl(EVENT_SESSION_ESTABLISHED);
	event->ptr = htobe64(intptr_t(ptrForJava));
	this->size = sizeof((queued_event_t *) 0)->type
			+ sizeof((queued_event_t *) 0)->ptr;
	return this->size;
}

int Events::writeOnNewSessionEvent(char *buf, void *ptrForJava,
		void *serverSession, struct xio_new_session_req *req) {
	struct queued_event_t* event = (struct queued_event_t*) buf;

	event->type = htonl(EVENT_SESSION_NEW);
	event->ptr = htobe64(intptr_t(ptrForJava));
	event->event_specific.new_session.ptr_session = htobe64(
			intptr_t(serverSession));
	event->event_specific.new_session.uri_len = htonl(req->uri_len);

	//copy data so far
	this->size = sizeof((queued_event_t *) 0)->type
			+ sizeof((queued_event_t *) 0)->ptr
			+ sizeof((struct event_new_session *) 0)->ptr_session
			+ sizeof((struct event_new_session *) 0)->uri_len;
...
int Events::writeOnMsgErrorEventServer(char *buf, void *ptrForJavaMsg,
		void* ptrForJavaSession, enum xio_status error) {
	struct queued_event_t* event = (struct queued_event_t*) buf;
	...
	event->event_specific.msg_error_server.ptr_session = htobe64(
			intptr_t(ptrForJavaSession));
	this->size = sizeof(struct event_msg_error_server)
			+ sizeof((queued_event_t *) 0)->type
			+ sizeof((queued_event_t *) 0)->ptr;
	return this->size;
}

int Events::writeOnMsgErrorEventClient(char *buf, void *ptrForJavaMsg,
		enum xio_status error) {
	struct queued_event_t* event = (struct queued_event_t*) buf;
	event->type = htonl(EVENT_MSG_ERROR_CLIENT);
	event->ptr = htobe64(intptr_t(ptrForJavaMsg));
	event->event_specific.msg_error_client.error_reason = htonl(error);
	this->size = sizeof(struct event_msg_error_client)
			+ sizeof((queued_event_t *) 0)->type
			+ sizeof((queued_event_t *) 0)->ptr;
	return this->size;
}

int Events::writeOnRequestReceivedEvent(char *buf, void *ptrForJavaMsg,
		const int32_t msg_in_size, const int32_t msg_out_size,
		void *ptrForJavaSession) {
	struct queued_event_t* event = (struct queued_event_t*) buf;
	...
	event->event_specific.req_received.ptr_session = htobe64(
			intptr_t(ptrForJavaSession));
	this->size = sizeof(struct event_req_received)
			+ sizeof((queued_event_t *) 0)->type
			+ sizeof((queued_event_t *) 0)->ptr;
	return this->size;
}

int Events::writeOnResponseReceivedEvent(char *buf, void *ptrForJavaMsg,
		const int32_t msg_in_size) {
	struct queued_event_t* event = (struct queued_event_t*) buf;
	event->type = htonl(EVENT_REPLY_RECEIVED);
	event->ptr = htobe64(intptr_t(ptrForJavaMsg));
	event->event_specific.res_received.msg_size = htonl(msg_in_size);
	this->size = sizeof(struct event_res_received)
			+ sizeof((queued_event_t *) 0)->type
			+ sizeof((queued_event_t *) 0)->ptr;
	return this->size;
}
...
int Events::writeOnFdReadyEvent(char *buf, int fd, int epoll_event) {
	...
	this->size = sizeof(struct event_fd_ready)
			+ sizeof((queued_event_t *) 0)->type
			+ sizeof((queued_event_t *) 0)->ptr;
	return this->size;
}
