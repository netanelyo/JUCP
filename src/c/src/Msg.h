...
public:
	...
	struct xio_msg* get_mirror_xio_msg() {
		return &xio_msg_mirror;
	}
	int send_response(const int size);
	int get_in_size() {
		return this->in_buf_size;
	}
	int get_out_size() {
		return this->out_buf_size;
	}
	bool was_assign_called() {
		return this->assign_called;
	}
...
inline const int get_xio_msg_in_size(struct xio_msg *msg) {
	...
	const int msg_in_size =
			(msg->in.data_iov.nents > 0) ? sglist[0].iov_len : 0;
	return msg_in_size;
}

inline const int get_xio_msg_out_size(struct xio_msg *msg) {
	struct xio_iovec_ex *sglist = vmsg_sglist(&msg->out);
	const int msg_out_size =
			(msg->out.data_iov.nents > 0) ? sglist[0].iov_len : 0;
	return msg_out_size;
}
