...
MsgPool::MsgPool(int msg_num, int in_size, int out_size) {
	...
	if (xio_mem_alloc(buf_size, &this->reg_mem)) {
		MSGPOOL_LOG_ERR(
				"allocating & registering memory failed with xio_mem_alloc(buf=%p, buf_size=%d) (errno=%d '%s')",
				this->reg_mem, this->buf_size, xio_errno(),
	...
	for (int i = 0; i < msg_num; i++) {
		msg = new Msg((char*) buf + i * (in_size + out_size), this->reg_mem.mr,
	...
	MSGPOOL_LOG_DBG(
			"CTOR done. allocated msg pool: num_msgs=%d, in_size=%d, out_size=%d",
			msg_num, in_size, out_size);
	return;
}
...
MsgPool::~MsgPool() {
	...
	if (xio_mem_free(&this->reg_mem)) {
		MSGPOOL_LOG_DBG("Error xio_mem_free failed: '%s' (%d)",
