...
int on_msg_callback_client(struct xio_session *session, struct xio_msg *msg,
	...
	LOG_TRACE(
			"on_msg_callback client=%p, num_iov=%d, len: in=%d out=%d, msg=%p",
	...
	int sizeWritten = ctx->events.writeOnResponseReceivedEvent(buf,
			msg->user_context, msg_in_size);
...
int on_msg_error_callback_client(struct xio_session *session,
	...
	int sizeWritten = ctx->events.writeOnMsgErrorEventClient(buf,
			msg->user_context, error);
	ctx->done_event_creating(sizeWritten);
...
int on_session_event_callback_client(struct xio_session *session,
	...
	LOG_DBG(
			"got on_session_event_callback. event=%d, cb_prv_data=%p, session=%p, conn=%p",
	...
	if (ctx) {
		...
		char* buf = ctx->get_buffer();
		int sizeWritten = ctx->events.writeOnSessionErrorEvent(buf, client,
				event_data);
		ctx->done_event_creating(sizeWritten);
	}
	return 0;
