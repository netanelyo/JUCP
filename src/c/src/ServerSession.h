...
public:
	...
	//inline functions
	void set_portal(ServerPortal *p) {
		this->forwardee = p;
	}
	struct xio_session* get_xio_session() {
		return session;
	}
	bool get_is_closing() {
		return is_closing;
	}
	void set_is_closing(bool b) {
		is_closing = b;
	}
	...
	Context *get_ctx_forwarder() {
		return ctx_forwarder;
	}
	...
	ServerPortal * get_portal_session_event(void * conn_user_context,
			struct xio_connection* con, enum xio_session_event event);
